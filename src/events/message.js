...
module.exports = async (client, message) => {
  ...
  // ////////////// Cooldowns ////////////////
  const delay = cmd.conf.cooldown ? cmd.conf.cooldown * 1000 : 300;
  ...
  if (cooldown[message.author.id] && cooldown[message.author.id][command]) {
    if (cooldown[message.author.id][command] && !cooldown[message.author.id][command].prompted) {
      ...
      const remaining = (delay - (new Date() - cooldown[message.author.id][command].timestamp)) / 1000;
  ...
  setTimeout(() => {
    ...
    if (Object.keys(cooldown[message.author.id]).length === 0) {delete cooldown[message.author.id]};
  }, delay);
